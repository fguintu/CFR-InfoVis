...
export function initS3(groups) {
  ...
  if (s3.length >= 2) {
    ...
    document.getElementById("callout3").textContent =
      `${best.race} students complete at ${fmtPct(best.rate)}, while ${worst.race} students complete at ${fmtPct(worst.rate)} — click two bars to compare ratios.`;
  }
  drawS3(s3);
}
...
function drawS3(data) {
  ...
  const W = 600, H = 380, M = { t: 16, r: 16, b: 44, l: 50 },
    w = W - M.l - M.r, h = H - M.t - M.b;

  const svg = d3.select("#vis-completion")
    .append("svg")
  ...
  function handleSelection(d, element) {
    ...
    selectedBars.push(d);
    d3.select(element).attr("stroke", "#333").attr("stroke-width", 2).attr("opacity", 1);

    if (selectedBars.length === 2) {
      const [first, second] = selectedBars;
      const ratio = (first.rate / second.rate).toFixed(2);
      
      callout.innerHTML = `<b>${first.race}</b> completion rates are <b>${ratio}x</b> those of <b>${second.race}</b> students. <span style="font-size:0.9em; font-weight:normal;">(Click another bar to reset)</span>`;
    } else {
