...
export function initS4(groups) {
  ...
  const repayShare = { hi: 0.15, md: 0.10, lo: 0.06, fg: 0.05 };
...
function drawS4(data) {
  ...
  const container = d3.select("#vis-racetrack");
  ...
  speedWrap.append("input")
    .attr("type", "range")
    .attr("min", 0.25).attr("max", 3).attr("step", 0.25).attr("value", 1)
    .on("input", function () {
      speedMul = +this.value;
  ...
  container.append("div").attr("class", "microcopy")
    .html("Each car\u2019s speed reflects how quickly that group can become debt-free. " +
  ...
  // Outer border
  const laneW = 28;
  svg.append("path").attr("d", trackD)
    .attr("fill", "none").attr("stroke", "#e5e0d8").attr("stroke-width", laneW * 2 + 2);

  // Inner fill
  const innerPts = [];
  for (let i = 0; i < N_PTS; i++) {
    const a = (2 * Math.PI * i) / N_PTS - Math.PI / 2;
    innerPts.push([cx + (RX - laneW) * Math.cos(a), cy + (RY - laneW) * Math.sin(a)]);
  }
  svg.append("path").attr("d", lineGen(innerPts))
    .attr("fill", "#faf8f4").attr("stroke", "none");

  // Track surface
  svg.append("path").attr("d", trackD)
    .attr("fill", "none").attr("stroke", "#ece8e0").attr("stroke-width", laneW * 2);

  // Center-line (dashed)
  svg.append("path").attr("d", trackD)
    .attr("fill", "none").attr("stroke", "#d5d0c8").attr("stroke-width", 1)
    .attr("stroke-dasharray", "8,6");

  // Hidden path for getPointAtLength
  const trackPath = svg.append("path").attr("d", trackD)
  ...
  svg.append("line")
    .attr("x1", finPt.x).attr("y1", finPt.y - laneW)
    .attr("x2", finPt.x).attr("y2", finPt.y + laneW)
    .attr("stroke", "#8b2500").attr("stroke-width", 2)
    .attr("stroke-dasharray", "4,3");
  svg.append("text")
    .attr("x", finPt.x).attr("y", finPt.y - laneW - 8)
    .attr("text-anchor", "middle")
    .attr("fill", "#8b2500").attr("font-size", "9px")
    .attr("font-family", "Helvetica,Arial,sans-serif")
    .attr("letter-spacing", "1.5px").attr("font-weight", "700")
    .text("START / LAP LINE");
  ...
  const scoreRows = scoreG.selectAll(".score-row")
    .data(cars).enter().append("g")
    .attr("class", "score-row")
    .attr("transform", (d, i) => `translate(0,${-30 + i * 22})`);
  scoreRows.append("rect")
    .attr("x", -8).attr("y", -7).attr("width", 16).attr("height", 14).attr("rx", 3)
    .attr("fill", d => d.color).attr("opacity", 0.8);
  scoreRows.append("text")
  ...
  cars.forEach((c, i) => { c.dist = i * 50; });

  // ── Car groups ──
  const carGs = svg.selectAll(".car-group")
    .data(cars).enter().append("g").attr("class", "car-group")
  ...
  carGs.append("rect")
    .attr("x", -16).attr("y", -9)
    .attr("width", 32).attr("height", 18).attr("rx", 7)
  ...
  carGs.append("text")
    ...
    .attr("fill", d => d.color).attr("font-size", "8px").attr("font-weight", "600")
    .text(d => d.label);
  ...
  function unpin() {
    if (pinnedCar) { pinnedCar.pinned = false; pinnedCar = null; }
  ...
  function pinCar(d) {
    ...
    panelDiv.html(
     ...
     .style("left", "10px").style("bottom", "10px");
  ...
  data.forEach(d => {
    ...
    item.append("div").attr("class", "legend-sw").style("background", d.color);
    item.append("span").text(`${d.label} (~${d.years.toFixed(0)} yrs)`);
  });
  ...
  function resetRace() {
    if (_timer) { _timer.stop(); _timer = null; }
    cars.forEach((c, i) => { c.dist = i * 50; c.lap = 0; });
  ...
  function startRace() {
    ...
    _timer = d3.timer((elapsed) => {
      if (lastT === null) { lastT = elapsed; }
      if (paused) { lastT = elapsed; return; }
      ...
      carGs.each(function (d) { positionCar(d3.select(this), d); });
